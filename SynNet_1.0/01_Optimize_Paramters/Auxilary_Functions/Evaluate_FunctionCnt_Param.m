...
function [Result, IsVague, RowValues, IsVagueRow] = Evaluate_FunctionCnt_Param(FunctionArray ,X, IsVagueX,Params)
% Params = [
...
Continuous_Circuit_F1C      = Params(1);
Continuous_Circuit_F2C      = Params(2);
Continuous_Circuit_Tmax     = Params(3);
Continuous_Circuit_FF4max   = Params(4);
Continuous_Circuit_OUTmax   = Params(5);
...
for K = size(FunctionArray,1):-1:1
    ...
    for i = 1:MaxAnd
        Filt = Fpos(i,:)>0;
        if any(Filt)
            RowValues(i,:)= F1(X(Function(i, Filt),:),Continuous_Circuit_F1C);
        end
    end
    mirFF4 = Continuous_Circuit_FF4max* F2(sum(RowValues,1), Continuous_Circuit_F2C, Continuous_Circuit_Tmax);
        %% handle the NOT part
    Result(K,:) = Continuous_Circuit_OUTmax * F1([mirFF4;X(-Function(Fneqs),:)],Continuous_Circuit_F1C);
end
