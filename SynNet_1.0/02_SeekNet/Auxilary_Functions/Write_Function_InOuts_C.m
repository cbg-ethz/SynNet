function Write_Function_InOuts_C(Function, Data, Results, FigurePath)
...
% D = Data.Values(Genes,:) - log2(Data.Threshold);
% Os = nan(Ns,1);
% if Ng>1
...
[~, SampleOrder] = sort(Results.Output_C, 'Ascend');

D2plot = Data.Values (Genes,SampleOrder);

D2plot = log2(D2plot);
D2plot = D2plot - log2(Data.Threshold);
...
CO = (Results.Output_C)*log2(10); % this is in log 10 already!
COm = (mean(CO(A)) + mean(CO(~A)))/2;
CO  = CO - COm;
CO  = CO / Max_Range;
...
imagesc((A(SampleOrder)-.5)*Max_Range*2);
% caxis(CRange);
set(gca, 'XTick', []);
set(gca, 'YTick', []);
xlim(XLs);
